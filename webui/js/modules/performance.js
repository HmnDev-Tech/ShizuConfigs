...
const PerformanceModule = (() => {
  function render() {
    return `
      <h5><i class="material-symbols-outlined" style="vertical-align:middle">speed</i> Performance</h5>
      <p style="color:var(--on-surface-variant)">Battery, CPU, Memory, and Temperature</p>
      ...
      <div id="perf-content">
          ...
          <article class="no-padding">
            <div class="padding">
              <div class="row no-wrap" style="align-items:center;gap:10px;margin-bottom:12px">
                ...
                <div><h6 class="small no-margin">Battery</h6><div class="small" id="perf-bat-level" style="color:var(--on-surface-variant)">—</div></div>
              </div>
          ...
          <article class="no-padding">
            <div class="padding">
              <div class="row no-wrap" style="align-items:center;gap:10px;margin-bottom:12px">
                ...
                <div><h6 class="small no-margin">Temperature</h6><div class="small" id="perf-thermal-status" style="color:var(--on-surface-variant)">—</div></div>
              </div>
        ...
        <h6 class="small" style="margin:20px 0 10px;color:var(--on-surface-variant)"><i class="material-symbols-outlined" style="font-size:18px;vertical-align:middle">developer_board</i> CPU</h6>
        ...
        <h6 class="small" style="margin:20px 0 10px;color:var(--on-surface-variant)"><i class="material-symbols-outlined" style="font-size:18px;vertical-align:middle">memory</i> Memory</h6>
        <article class="no-padding"><div class="padding" id="perf-mem-info"></div></article>

        <h6 class="small" style="margin:20px 0 10px;color:var(--on-surface-variant)"><i class="material-symbols-outlined" style="font-size:18px;vertical-align:middle">bolt</i> Actions</h6>
        <div class="row" style="gap:8px">
          <button class="border" onclick="PerformanceModule.action('cache')"><i class="material-symbols-outlined">delete_sweep</i> Drop Cache</button>
          <button class="border" onclick="PerformanceModule.action('refresh')"><i class="material-symbols-outlined">refresh</i> Refresh</button>
        </div>
  ...
  function loadBattery() {
    ...
    lines.forEach(l => {
      ...
      if (t.startsWith('status:')) { const s=parseInt(t.split(':')[1]); status=['?','Charging','Discharging','Not charging','Full'][s]||s; }
    ...
    document.getElementById('perf-bat-info').innerHTML = `
      <div class="stat-grid">
        <div class="stat-item"><div class="stat-label">Temperature</div><div class="stat-value">${temp}</div></div>
        <div class="stat-item"><div class="stat-label">Voltage</div><div class="stat-value">${volt}</div></div>
        <div class="stat-item"><div class="stat-label">Technology</div><div class="stat-value">${tech}</div></div>
        <div class="stat-item"><div class="stat-label">Source</div><div class="stat-value">${powered||'Battery'}</div></div>
      </div>
  ...
  function loadThermal() {
    ...
    document.getElementById('perf-thermal-info').textContent = r.ok ? r.stdout.trim() : 'Unavailable';
    const m = r.ok ? r.stdout.match(/Thermal Status:\s*(\d+)/) : null;
    document.getElementById('perf-thermal-status').textContent = m ? ['Normal','Light','Moderate','Severe','Critical','Emergency','Shutdown'][parseInt(m[1])]||m[1] : '—';
  }

  function loadCPU() { const r = ShellBridge.exec('cat /proc/cpuinfo'); document.getElementById('perf-cpu-info').textContent = r.ok ? r.stdout.trim() : 'Unavailable'; }
  ...
  function action(type) {
    ...
    if (type==='cache') { const r=ShellBridge.exec('echo 3 > /proc/sys/vm/drop_caches 2>&1 || echo "Root required"'); out.textContent=r.ok?r.stdout:r.stderr; }
    else if (type==='refresh') { loadAll(); out.textContent='Updated'; }
